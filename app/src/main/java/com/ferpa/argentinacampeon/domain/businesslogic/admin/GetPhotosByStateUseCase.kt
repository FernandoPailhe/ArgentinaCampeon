...
class GetPhotosByStateUseCase @Inject constructor(
    private val repository: PhotoRepository
) {

    operator fun invoke(rarity: Int): Flow<Resource<List<PhotoDto>>> = flow {
        try {
            emit(Resource.Loading())
            val photos = repository.getPhotosByState(rarity)
